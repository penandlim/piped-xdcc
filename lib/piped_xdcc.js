...
function pipeXdccRequest(client, packInfo, callback) {
  ...
  function handleMessage(sender, target, message, raw) {
    if (target !== client.nick) {
      return;
    }

    if (message == "VERSION") {
      console.log("asked me about version... how do i respond?");
      client.say(sender, "VERSION deepdarkfantasy");
    }
    ...
    if (messageInfo.port > 65536 || !(/^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/).test(messageInfo.ip)) {
        cleanError("Cannot connect to IPv6 bots. Use a different bot.");
        return;
    }
  ...
  function cleanError(message) {
    client.removeListener("no-such-nick", handleBotMissing);
